...
def bounded_schedule_cases(
    ...
    *,
    ...
    ordering, dangling-direction, and convergence problems become measurable.
    """
    ...
    if not ordered_waves or any(not isinstance(wave, int) or isinstance(wave, bool) or wave < 0 for wave in ordered_waves):
...
def _unresolved_external_writes(receipt: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    direction flags that accidentally remain true after convergence.
    """
    ...
    for index, transition in enumerate(transitions):
        ...
        for key, value in transition.get("writes", {}).items():
            overwritten = any(key in later.get("writes", {}) for later in transitions[index + 1 :])
...
def explore_schedule_space(
    ...
    *,
    ...
    for case in cases:
        ...
        orphan_keys = sorted(key for key in applied_external_write_keys if key not in module_read_key_set)
    ...
    atlas = {
        "schema": "axm.causal-loop.causal-atlas/v0.05",
        ...
        "summary": {
            ...
            "hardInvariantFailureCaseCount": sum(bool(result["hardInvariantFailures"]) for result in results),
            "contradictionCaseCount": sum(result["contradictionCount"] > 0 for result in results),
